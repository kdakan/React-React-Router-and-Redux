...
* Handling state with setState() inside a component is simple, however when there is a component that depends on another component's state, things get complicated. Without using a state management library like Redux or Mobx, a common way to share state between components is to lift the state up to a common ancestor component, put the state changing code on this ancestor component and share the code (callback function) and shared state down the tree to the other components, and inside the child components, bind to this shared state passed on props, and call the callback function passed on props to change this state and trigger other components which depend on this state. As the app grows larger, this way of sharing state leads to unmaintainable complex code and it is hard to find dependent components inside the codebase.
* When starting a new app or a new component tree, it is easier to start without thinking about any state and just break down the target HTML outout we want, into visual dumb components. The root component of the app or the component tree can be fed through its props a static data structure to prototype the ui.
* Next, we should identify the changing parts of the data and turn these parts into state. How we implement state is different whether we are using a state management library or not. In any case, we do not put derived values into their own state, instead we use functions, getters, or in case of MobX, @computed functions to compute derived state from true state.
...
* If we are using a state management library (meaning that if our app is not very small), then we need to place this state into a store or multiple stores and pass the store to the component, and fire actions on DOM events without using setState()
...
And inside an event handler, we do not use setState() to change state, instead, we change it like changing a regular javascript primitive value, object or array. We also mark the component or components which uses/depends on this state variable with the @observer annotation.
